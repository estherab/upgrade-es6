...
const result = videogames
  ...
  .reduce((acumulador, current, index, array) => {
    acumulador.sum += current.score;
    acumulador.items++;
    
    if (index === array.length - 1) { // Averiguando la última iteración
      acumulador = acumulador.sum / acumulador.items; // 7.875
    };

    return acumulador;
  }, {sum: 0, items: 0});

console.log("Media juegos RPG:", result);
